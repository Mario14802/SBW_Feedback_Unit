...
Function CalculateCurrent(ADCValue As Double, Vref As Double, Gain As Double, R_shunt As Double) As Double
    ' Calculate the current using the formula:
    ' I = (ADCValue * Vref) / (4095 * Gain * R_shunt)
    CalculateCurrent = (ADCValue * Vref) / (4095 * Gain * R_shunt)
End Function
